        ...
        <div class="topbar"> 
            ...
            <div class="mobile-search-form" id="mobileSearchForm">
                <form class="w-100" onsubmit="event.preventDefault();">
                    <div class="input-group">
                        <input type="text" class="form-control border-0 mobile-search-input" placeholder="Cari anak didik..." autocomplete="off" style="border-radius: 20px 0 0 20px;">
            ...
            <div class="d-flex align-items-center">
                <form class="search-form d-none d-md-block me-3" onsubmit="event.preventDefault();">
                    <input type="text" class="form-control" placeholder="Type for search..." id="childSearchInput" autocomplete="off">
                </form>
            ...
            <div class="row g-4" id="childList">
                <?php if(count($daftar_anak) > 0): ?>
                    <?php foreach($daftar_anak as $anak): ?>
                        <div class="col-md-6 col-xl-4 child-item-wrapper">
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#closeSearch').on('click', function() {
                ...
                // Clear mobile search and trigger search to reset list
                $('.mobile-search-input').val('').trigger('input');
            });

            // Live Search Script - Responsive (input event)
            $('#childSearchInput, .mobile-search-input').on('input', function() {
                var value = $(this).val().toLowerCase();
                $('#childList .child-item-wrapper').each(function() {
                    $(this).toggle($(this).text().toLowerCase().indexOf(value) > -1);
                });
            });
            
            // Handle cross-page search if query param exists
            const urlParams = new URLSearchParams(window.location.search);
            const searchVal = urlParams.get('search');
            if(searchVal) {
                $('#childSearchInput').val(searchVal).trigger('input');
            }
        });
    </script>
</body>
